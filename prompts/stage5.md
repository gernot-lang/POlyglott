# POlyglott Stage 5: DeepL Integration

## Overview

Add a `translate` subcommand that uses the DeepL API to machine-translate entries in the master CSV. This stage connects the translation workflow: the master CSV (Stage 4) identifies what needs translating, the glossary (Stage 2) protects key terms, context (Stage 3) improves translation quality, and DeepL provides the translations.
...
Arguments:
  MASTER_CSV                 Path to master CSV file (polyglott-accepted-<lang>.csv)
...
Inferred from master CSV filename, same as Stage 4: `polyglott-accepted-de.csv` → `DE` for DeepL.
...
- Existing `scan`, `lint` tests still pass
...
- Tag: `git tag -a v0.5.0 -m "Add DeepL integration for machine translation"`
